...
export async function feed(req,res) {
    try{
        ...
        `WHERE sqrt((("lat" - $1) * 111)^2 + (("lon" - $2) * 111)^2) < $3 AND ("delivery" = false OR (sqrt((("lat" - $1) * 111)^2 + (("lon" - $2) * 111)^2) < "delivery-range" AND "delivery"=true)) ${tagsQuery} ${deliveryQuery} ${searchQuery} AND "time" <= $4 ORDER BY "time" DESC,"specialId" LIMIT $5 OFFSET $6`,
        ...
        if(result.rows.length && result.rows.length > 0){
            ...
            for(let i = 0; i < result.rows.length; i++){
                ...
                if(result.rows[i]["working-hours-from"] !== null) 
                {
                    ...
                    if((clientDateAndTime < result.rows[i]["working-hours-from"] || clientDateAndTime > result.rows[i]["working-hours-to"]) && (result.rows[i]["working-hours-from"] !== result.rows[i]["working-hours-to"])){
